...
function WelcomeBackdrop() {
  return (
    <View pointerEvents="none" style={StyleSheet.absoluteFill}>
      <LinearGradient
        colors={["rgba(125,211,252,0.28)", "rgba(125,211,252,0)", "rgba(59,130,246,0.22)"]}
        end={{ x: 1, y: 1 }}
        start={{ x: 0, y: 0 }}
        style={StyleSheet.absoluteFill}
      />
      <View style={[styles.glowSky, styles.decor]} />
      <View style={[styles.glowOcean, styles.decor]} />
...
export function PrincipalWelcomeCard() {
  ...
  const name = profile?.fullName ?? session?.principal.name ?? "Müdür";
  const schoolName = tenantQ.data?.name ?? "Okulunuz";
  const accent = profile?.profileAccent ?? "#2563eb";
  ...
  return (
    <View style={styles.shell}>
      <Pressable
        ...
        onPress={() => router.push("/(app)/principal/profile")}
        style={({ pressed }) => [pressed && styles.cardPressed]}
      >
        <LinearGradient
          colors={["#38bdf8", "#0ea5e9", "#2563eb", "#1d4ed8"]}
          end={{ x: 1, y: 1 }}
          locations={[0, 0.34, 0.68, 1]}
          start={{ x: 0, y: 0 }}
          style={[
            styles.card,
            platformShadow("0 20px 44px rgba(37,99,235,0.28)", {
              shadowColor: "#2563eb",
